...
const makeResizable = (dialog: HTMLDivElement) => {
  dialog.style.resize = 'both'
  dialog.style.transformOrigin = 'top left'
  dialog.style.overflow = 'auto'
}

const makeDraggable = (dialog: HTMLDivElement, handle: HTMLDivElement) => {
  let offsetX = 0
  let offsetY = 0
...
class NotePlus extends LiteGraph.LGraphNode {
  ...
  hookResize(elem) {
    if (!this.resizeObserver) {
      infoLogger('Setting up size observer for the dialog')
      const observer = () => {
  ...
  setupEditors() {
    ...
    // Ensure ACE is loaded before trying to use it
    if (typeof ace === 'undefined' || !ace.edit) {
      errorLogger('ACE editor not loaded. Cannot set up editors.')
      return
    }

    // Only create new editor instances if they don't already exist
    if (!this.html_editor) {
    ...
    this.setMode(DEFAULT_MODE) // This will also update html_editor mode if it exists

    // Set values every time, as the dialog content is recreated
    this.html_editor.setValue(this.html_widget.value, -1) // -1 moves cursor to start
    this.css_editor.setValue(this.css_widget.value, -1)
  }
  ...
  setupDoubleClickEdit() {
    // Attach to the main widget element, which contains the 'inner' div
    this.html_widget.element.addEventListener('dblclick', (e) => {
      // Prevent LiteGraph's default dblclick behavior if any
      e.stopPropagation()
      e.preventDefault()
  ...
  enterEditMode() {
    ...
    this.quickEditorContainer = document.createElement('div')
    this.quickEditorContainer.id = `noteplus-quick-editor-${this.uuid}` // Unique ID for Ace
    Object.assign(this.quickEditorContainer.style, {
      ...
      right: '0',
      bottom: '0',
      zIndex: '10', // Ensure it's on top of other elements within the widget
      // Ace theme will handle background/colors
    })
    ...
    this._setupEditor(this.quickEditor) // Apply common settings
    this.quickEditor.session.setMode('ace/mode/markdown') // Ensure markdown mode
    this.quickEditor.setTheme(`ace/theme/${this.theme_widget.value}`) // Apply current theme

    this.quickEditor.setValue(this._raw_html, -1) // Set content, cursor at end
    this.quickEditor.focus()

    // Live update on change
    this.quickEditor.session.on('change', () => {
      if (this.live) {
        // Only live update if enabled
        this.updateHTML(this.quickEditor.getValue())
      }
    })

    // Handle blur (save) and keydown (Enter/Escape)
    // Ace editor's textInput element is the actual textarea it uses
    ...
    this.quickEditor.textInput.getElement().addEventListener('keydown', (e) => {
      if (e.key === 'Enter' && !e.shiftKey) {
        ...
        this.exitEditMode(true) // Save on Enter
      } else if (e.key === 'Escape') {
        e.preventDefault()
        this.exitEditMode(false) // Discard on Escape
      }
    })

    // Resize the editor to fit its container
    this.quickEditor.resize()
    this.setDirtyCanvas(true, true) // Force LiteGraph to redraw and potentially resize
  }
  ...
  exitEditMode(saveChanges) {
    ...
    if (this.quickEditor) {
      if (saveChanges) {
        ...
        if (newValue !== this._raw_html) {
          ...
          this.updateHTML(newValue) // Re-render the markdown
        }
      }
      this.quickEditor.destroy() // Clean up Ace instance
    ...
    this.inner.style.display = '' // Show the preview div again
    this.setDirtyCanvas(true, true)
    this.calculateHeight() // Recalculate height based on rendered content
  }
}
